...
blastparams_vch = Channel.fromPath(params.blastParams)
...
process blastSimilarity {
   ...
   path 'blastParams' from blastparams_vch
   output:
   path 'blastSimilarity.out' into output_qch
   """
   blastSimilarity --regex '(S+)' --pValCutoff 1e-5 --lengthCutoff 10 --percentCutoff 20 --blastBinDir /usr/bin/ncbi-blast-2.13.0+/bin --blastProgram blastp --database /work/newdb.fasta --seqFile subset.fa  --blastParams blastParams --blastVendor ncbi
   """
}
